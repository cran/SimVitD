...
The package \pkg{SimVitD} presents simulation based tools to aid planning the comparison of supplementation schemes in vitamin D studies, which aim to account for a set of perceived sources of variability and heterogeneity in vitamin D benefit. As seasonal variation in solar radiation is pronounced, there will be a natural, sinoid variation in vitamin D status; consequently, the relative contribution of vitamin D supplementation to the overall vitamin D status, and it's impact, will vary seasonally: while vitamin D supplementation may contribute the majority of the vitamin D in Winter, the same dose may be relatively insignificant in the Summer. Some of the \pkg{SimVitD} schemes are shown in Figures~\ref{fig:fixed-dose} and~\ref{fig:dynamic-dose}. \newline
...
\hspace*{-1cm}\fbox{
\begin{tikzpicture}
  \tikzstyle{sta} = [rectangle, rounded corners,draw,minimum width=20pt]
	
\node[text width = 90pt] (propw) at (3,5.2) {\bf{Carry out a test of $H_0$ for each study}};

\node[text width = 90pt] (prop) at (-1,5.2) {\bf{Simulate many instances of the study}};
	
	\node[text width = 120pt] (propr) at (7.5,5.2) {\bf{Combine all decisions to approximate power}};
	
	
	\node[text width = 120pt] (propq) at (-6,5.2) {\bf{Specify settings for simulation of indivuals in schemes $A$ and $B$} };	

	\node[sta,text width = 120pt] (orig1) at (-6,1) {Give $n$ and settings for simulation
	of vitamin D profiles under schemes $A$ and $B$, exposures rate function $\lambda(t)$, $p_0$, $g(x)$, hypotheses to test $H_0,H_A$ and $\alpha$.  }; 
	
\node[sta,text width = 110pt] (origend) at (7,1) { Approximate the power via \[\widehat{\mbox{Power}} = \frac{\mbox{\# $H_0$ rejected}}{N}\] }; 	
	
	
	%\node (orig1) at (-5,1) {};
	\node[sta] (mv1) at (-1.5,3) {Study 1}; 
  	\node[sta] (mv2) at (-1.5,1.5) {Study 2}; 
  	\node[sta] (mv3) at (-1.5,-1) {Study $N$}; 
  	
  	\node (mvd) at (-1,0.5) {$\vdots$};
  	\node (mvd) at (-1,0) {$\vdots$};
  	
  	\node[sta] (mvv1) at (2,3) {$H_0$ decision 1}; 
  	\node[sta] (mvv2) at (2,1.5) {$H_0$ decision 2}; 
  	\node[sta] (mvv3) at (2,-1) {$H_0$ decision $N$}; 
  	
  	\node (mvd) at (2.5,0.5) {$\vdots$};
  	\node (mvd) at (2.5,0) {$\vdots$};
  	
  	%\draw[-] (orig) edge (orig1);
  	\draw[->]   (orig1) edge (mv1);
  	\draw[->]  	(orig1) edge (mv2);
  	\draw[->] 	(orig1) edge (mv3);
  	...
  	\draw[->]   (mvv1) edge (origend);
  	\draw[->]  	(mvv2) edge (origend);
  	\draw[->] 	(mvv3) edge (origend);
